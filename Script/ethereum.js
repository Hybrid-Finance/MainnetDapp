...
async function getBalance() {
    ...
    document.getElementById("dailyReward").innerHTML = (format * dailyPercent).toFixed(2);
}
        ...
        if (error.code === 4902) {
            try {
                await window.ethereum.request({
                    ...
                    params: [{
                        chainId: '0xa86a',
                        chainName: 'Avalanche Mainnet Network',
                        nativeCurrency: {
                            name: 'AVAX',
...
async function addTokenFunction() {
    ...
    try {
        ...
        if (wasAdded) {
            console.log('Hybrid Finance token was added');
        } else {
            console.log('Hybrid Finance token has not been added');
        }
    } catch (error) {
        console.log(error);
    }
